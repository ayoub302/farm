...
const textos = {
  ...
  fr: {
    ...
    verTodasActividades: "Voir toutes les activités",
    fechaSeleccionada: "Date:",
    sinSeleccion: "Aucune date sélectionnée",
    limpiarSeleccion: "Effacer",
    cargando: "Chargement...",
    errorCargando: "Erreur de chargement",
    horario: "Horaire",
    verDetalles: "Voir détails",
    actividadesDia: "Activités du jour",
  },
};
...
export default function Calendario() {
  ...
  // Fetch actividades del calendario
  useEffect(() => {
          ...
          Object.keys(data.activitiesByDate).forEach((fecha) => {
            data.activitiesByDate[fecha].forEach((item) => {
              actividadesTransformadas.push({
                ...
                enlace: item.enlace || `/actividades?date=${fecha}`,
  ...
  const formatearFechaCorta = (fecha) => {
    if (!fecha) return "";
    return fecha.toLocaleDateString(language === "ar" ? "ar-EG" : "fr-FR", {
      day: "numeric",
      month: "short",
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6 md:mb-8">
        ...
        <h3 className="text-lg md:text-2xl font-bold text-gray-800 text-center px-2">
            ...
            <div className="flex-grow">
              ...
              {getActividadesDia(fechaSeleccionada).length > 0 && (
                <div>
                  ...
                  <div className="space-y-2 max-h-40 md:max-h-48 overflow-y-auto pr-2">
                        ...
                        >
                          <div className="flex items-start">
                            ...
                            <div className="flex-grow min-w-0">
                              ...
                              {/* Información en columnas en móvil, en fila en desktop */}
        ...
        {dias.map((fecha, index) => {
          ...
          return (
            <button
              ...
              className={`
                min-h-[60px] md:min-h-24 lg:min-h-32 xl:min-h-36 
                ...
                ${esFinDeSemana ? "text-gray-500" : "text-gray-800"}
              `}
            >
              {fecha && (
                <>
                  ...
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {esHoy && (
                      <span className="text-[10px] md:text-xs bg-green-500 text-white px-1 md:px-2 py-0.5 rounded">
                        {t.hoy}
                      </span>
                    )}
                  </div>

                  {/* Indicadores de actividades (móvil: puntos, desktop: mini cards) */}
                  <div className="flex-grow flex flex-col justify-center">
                    {/* En móvil: mostrar solo puntos indicadores */}
                    <div className="md:hidden flex flex-wrap justify-center gap-0.5 mt-1">
                      {actividades.slice(0, 4).map((act, idx) => (
                        <div
                          key={idx}
                      ...
                      {actividades.length === 0 && esFinDeSemana && (
                        <div className="text-[8px] text-blue-400">🌴</div>
                      )}
                    </div>

                    {/* En desktop: mostrar mini cards */}
                    <div className="hidden md:block space-y-1">
                      ...
                      {actividades.length === 0 && (
                        <div className="text-[10px] text-gray-400 text-center">
                          {esFinDeSemana ? t.finSemana : t.sinActividades}
                        </div>
                      )}
      ...
      <div className="mt-6 md:mt-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            href="/actividades"
            className="inline-block bg-green-600 text-white px-6 py-2.5 md:px-8 md:py-3 rounded-lg font-semibold hover:bg-green-800 transition text-sm md:text-base text-center"
          >
          ...
          {fechaSeleccionada && (
            <Link
              ...
              className="inline-block bg-blue-600 text-white px-6 py-2.5 md:px-8 md:py-3 rounded-lg font-semibold hover:bg-blue-800 transition text-sm md:text-base text-center"
            >
              {language === "ar" ? "احجز هذا اليوم" : "Réserver ce jour"}
            </Link>
          )}
        </div>
      </div>
